...
from SvgProcessing_CubiCasa import SVGParserCUBI
...
"""
This script is designed to process floorplan data from the CubiCasa5K dataset, specifically parsing SVG annotations and generating geometric primitives (e.g., walls, doors, and windows) while refining and partitioning labels

This script is the main code that processes dual graph according to VecFloorSeg paper.
wall_svg.png(Segmentation labels for walls) represents the primal graph and icon_svg.png(Segmentation labels for icons) represents dual graph
"""


reSize = 512 # Target size for resizing images (512 pixels).

REFER_NAME = 'wall_svg.png' # Name of the file that contains the segmentation labels for walls.
TARGET_DIR = 'CUBI_new_1' # Directory where processed images and labels will be saved.


def alterLabel(imgArray, lblArray):
    # Goal: For connected regions divided by lines in imgArray, perform vote operations for eacgh region and  assign the most frequent label from lblArray.
    ...
    mask = np.logical_and(BMask, GMask)
    mask = np.logical_and(mask, RMask).astype(np.uint8)
    
    # Apply mask to identify boundary regions
...
def worker(filePth, imgSaveDir, labelSaveDir):
    ...
    imgName = os.path.basename(os.path.dirname(filePth))
